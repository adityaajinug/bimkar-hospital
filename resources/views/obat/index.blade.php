            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('obat.create') }}" class="inline-block py-2 px-4 text-white bg-blue-700 rounded-lg hover:bg-blue-800 text-base transform transition duration-150 active:scale-90 focus:scale-95 mb-5">Tambah</a>
                    </div>
                        ...
                        <table class="table-auto w-full rounded-md">
                            <thead class="bg-gray-100 text-gray-700">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-left">Kemasan</th>
                            ...
                            <tbody>
                                @forelse ($data as $index => $row)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $row->nama_obat }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $row->kemasan }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">Rp {{ number_format($row->harga, 2) }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-center">
                                            ...
                                            <form action="{{ route('obat.destroy', $row->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-gray-500 py-4">Tidak ada data Obat.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
